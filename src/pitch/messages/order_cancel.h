#ifndef _LIBPITCH_PITCH_ORDER_CANCEL_H
#define _LIBPITCH_PITCH_ORDER_CANCEL_H
...
namespace pitch::messages
{
     ...
     *   canceled_shares 21        6         decimal            uint64_t
     *   LF              39
     *
    ...
    {
    public:
        order_cancel(uint64_t timestamp,
                  uint64_t _order_id,
                  uint64_t _shares) : message(timestamp),
                                     order_id(_order_id),
                                     canceled_shares(_shares) {}
        message_type get_type() { return message_type::order_cancel; }
        ...
        /**
         * number of shares cancel
         */
        uint64_t get_canceled_shares() { return canceled_shares; }


        private:
        uint64_t order_id;
        uint64_t canceled_shares;
    };


    /**
     * Class implementing builder pattern for order_cancel. All methods and data fields are private
     * as it is only meant to be called by friend pitch::decoder<T1>
    */
    template <typename T1>
    class _order_cancel_decoder
    {
    private:
        ...
        static T1 decode_message(T2 begin, T2 end, uint64_t timestamp)
        {
            ...
            return T1(new order_cancel(timestamp, order_id, shares));
        }
